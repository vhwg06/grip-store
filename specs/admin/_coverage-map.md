...
|---|---|---|---|---|
| `store-setting` | `005-admin-store-settings`, phần banner/about từ `007` và `004` | `src/app/admin/settings/page.tsx`, `getAdminDashboard()` | `Active + Spec-derived` | canonical owner của storefront-facing config |
...
| `content` | `004` và `007` historical evidence + active admin routes hiện có | `/admin/media`, `/admin/banners`, `/admin/articles`, `/admin/faqs`, `/admin/about`, `/admin/product-content` | `Active + Spec-derived` | editorial/content behavior không nên bị nhét vào settings hay product core |
...
- `store-setting` -> `content`: settings quyết định presence; content quyết định editorial substance
