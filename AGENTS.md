...
- Keep repeated reads, failed attempts, retries, and intermediate reasoning local to the subagent; return only useful conclusions, contracts, dependencies, and verification results.
- Prefer compact handoffs over reading another subagent's raw scope when the handoff is sufficient.
